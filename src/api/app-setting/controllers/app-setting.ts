    ...
    return {
        ...
        async createDefault(ctx) {
            try {
                ...
                const publishedSetting = await strapi.documents('api::app-setting.app-setting')
                    .publish({
                        documentId: newSetting.documentId,
                    });

                await logService.recordAppSettingLog(ctx, {
                    action: 'create',
                    details: 'Création des paramètres systèmes par défaut',
                    dataAfter: publishedSetting,
                });

                return ctx.send({ data: publishedSetting }, 201);
            } catch (error) {
        ...
        async updateDefault(ctx) {
            try {
                ...
                await logService.recordAppSettingLog(ctx, {
                    action: 'update',
                    details: 'Mise à jour des paramètres systèmes',
                    dataBefore: existingDefault,
                    dataAfter: publishedSetting,
                });

                return ctx.send({ data: publishedSetting }, 200);
            } catch (error) {
